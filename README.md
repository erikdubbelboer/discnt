[![Build Status](https://travis-ci.org/antirez/discnt.svg)](https://travis-ci.org/antirez/discnt)

Discnt, an in-memory, distributed job queue
===

Discnt is ongoing experiment to build a distributed, in memory, message broker.
...
Discnt is a distributed and fault tolerant message broker, so it works as
...
jobs, Discnt often uses the term "job" in the API and in the documentation,
however jobs are actually just messages in the form of strings, so Discnt
...
in the details. A few details about Discnt are:

Discnt is a **synchronously replicated job queue**. By default when a new job is added, it is replicated to W nodes before the client gets an acknowledge about the job being added. W-1 nodes can fail and still the message will be delivered.

Discnt supports both **at-least-once and at-most-once** delivery semantics. At least once delivery semantics is where most efforts were spent in the design and implementation, while the at most once semantics is a trivial result of using a *retry time* set to 0 (which means, never re-queue the message again) and a replication factor of 1 for the message (not strictly needed, but it is useless to have multiple copies of a message around if it will be delivered at most one time). You can have, at the same time, both at-least-once and at-most-once jobs in the same queues and nodes, since this is a per message setting.

Discnt at-least-once delivery is designed to **approximate single delivery** when possible, even during certain kinds of failures. This means that while Discnt can only guarantee a number of deliveries equal or greater to one, it will try hard to avoid multiple deliveries whenever possible.

Discnt is a distributed system where **all nodes have the same role** (aka, it is multi-master). Producers and consumers can attach to whatever node they like, and there is no need for producers and consumers of the same queue, to stay connected to the same node. Nodes will automatically exchange messages based on load and client requests.

Discnt is Available (it is an eventually consistent AP system in CAP terms): producers and consumers can make progresses as long as a single node is reachable.

Discnt supports **optional asynchronous commands** that are low latency for the client but provide less guarantees. For example a producer can add a job to a queue with a replication factor of 3, but may want to run away before knowing if the contacted node was really able to replicate it to the specified number of nodes or not. The node will replicate the message in the background in a best effort way.

Discnt **automatically re-queue messages that are not acknowledged** as already processed by consumers, after a message-specific retry time. There is no need for consumers to re-queue a message if it was not processed.

Discnt uses **explicit acknowledges** in order for a consumer to signal a message as delivered (or, using a different terminology, to signal a job as already processed).

Discnt queues only provides **best effort ordering**. Each queue sorts messages based on the job creation time, which is obtained using the *wall clock* of the local node where the message was created (plus an incremental counter for messages created in the same millisecond), so messages created in the same node are normally delivered in the same order they were created. This is not causal ordering since correct ordering is violated in different cases: when messages are re-issued because not acknowledged, because of nodes local clock drifts, and when messages are moved to other nodes for load balancing and federation (in this case you end with queues having jobs originated in different nodes with different wall clocks). However all this also means that normally messages are not delivered in random order and usually messages created first are delivered first.

Note that since Discnt does not provide strict FIFO semantics, technically speaking it should not be called a *message queue*, and it could better identified as a message broker. However I believe that at this point in the IT industry a *message queue* is often more lightly used to identify a generic broker that may or may not be able to guarantee order in all the cases. Given that we document very clearly the semantics, I grant myself the right to call Discnt a message queue anyway.

Discnt provides the user with fine-grained control for each job **using three time related parameters**, and one replication parameter. For each job, the user can control:

1. The replication factor (how many nodes have a copy).
2. The delay time (the min time Discnt will wait before putting the message in a queue, making the message deliverable).
...
Finally, Discnt supports optional disk persistence, which is not enabled by default, but that can be handy in single data center setups and during restarts.

ACKs and retries
---

Discnt implementation of *at least once* delivery semantics is designed in order
...
In order to avoid multiple deliveries when possible, Discnt uses client ACKs. When a consumer processes a message correctly, it should acknowledge this fact to Discnt. ACKs are replicated to multiple nodes, and are garbage collected as soon as the system believes it is unlikely that more nodes in the cluster have the job (the ACK refers to) still active. Under memory pressure or under certain failure scenarios, ACKs are eventually discarded.
...
As already mentioned, In order to control replication and retries, a Discnt job has the following associated properties: number of replicas, delay, retry and expire.
...
Discnt supports a faster way to acknowledge processed messages, via the
...
Discnt and disk persistence
---

Discnt can be operated in-memory only, using the synchronous replication as
...
Normally Discnt only reloads jobs data in memory, without populating queues,
...
Even when running memory-only, Discnt is able to dump its memory on disk and reload from disk on controlled restarts, for example in order to upgrade the software.
...
Discnt jobs are uniquely identified by an ID like the following:
...
in a cluster with 100 Discnt nodes, the probability of two nodes to have
...
1. Compile Discnt, if you can compile Redis, you can compile Discnt, it's the usual no external deps thing. Just type `make`. Binaries (`discnt` and `discnt-server`) will end up in the `src` directory.
...
For example if you are running three Discnt servers in port 7711, 7712, 7713 in order to join the cluster you should use the `discnt` command line tool and run the following commands:

    ./discnt -p 7711 cluster meet 127.0.0.1 7712
    ./discnt -p 7711 cluster meet 127.0.0.1 7713
...
Remember that you can add and get jobs from different nodes as Discnt
...
* *job* is a string representing the job. Discnt is job meaning agnostic, for it a job is just a message to deliver. Job max size is 4GB.
...
Claims to be still working with the specified job, and asks Discnt to postpone
...
Note that the `WORKING` command is refused by Discnt nodes if 50% of the job
time to live has already elapsed. This limitation makes Discnt safer since
...
Discnt uses the same protocol as Redis itself. To adapt Redis clients, or to use it directly, should be pretty easy. However note that Discnt default port is 7711 and not 6379.

While a vanilla Redis client may work well with Discnt, clients should optionally use the following protocol in order to connect with a Discnt cluster:
...
API on top of Discnt:

*C++*

- [discnt C++ client](https://github.com/zhengshuxin/acl/tree/master/lib_acl_cpp/samples/discnt)

*Elixir*

- [exdiscnt](https://github.com/mosic/exdiscnt)

*Erlang*

- [ediscnt](https://github.com/nacmartin/ediscnt)

*Go*

- [discnt-go](https://github.com/zencoder/discnt-go)
- [go-discnt](https://github.com/EverythingMe/go-discnt)
- [discnt](https://github.com/goware/discnt)

*Java*

- [jediscnt](https://github.com/xetorthio/jediscnt)
- [spinach](https://github.com/mp911de/spinach)

*Node.js*

- [discnt.js](https://www.npmjs.com/package/discnt.js)
- [thunk-discnt](https://github.com/thunks/thunk-discnt)

*Perl*

- [perl-discnt](https://github.com/lovelle/perl-discnt)

*PHP*

- [phpque](https://github.com/s12v/phpque) (PHP/HHVM)
- [discnt-php](https://github.com/mariano/discnt-php) ([Composer/Packagist](https://packagist.org/packages/mariano/discnt-php))
- [discnt-client-php](https://github.com/mavimo/discnt-client-php) ([Composer/Packagist](https://packagist.org/packages/mavimo/discnt-client))
...
- [pydiscnt](https://github.com/ybrs/pydiscnt) ([PyPi](https://pypi.python.org/pypi/pydiscnt))

*Ruby*

- [discnt-rb](https://github.com/soveran/discnt-rb)

*.NET*

- [Discnt.Net](https://github.com/ziyasal/Discnt.Net)
...
1. Discnt tries to replicate to W-1 (or W during out of memory) reachable nodes, shuffled.
...
Discnt is a full mesh, with each node connected to each other. Discnt performs
...
Being Discnt multi-master the event of nodes failing is not handled in any
special way.
...
describing the Discnt state machine, there is a more detailed description
...
4. `acknowledged`: the job is no longer actived since a client confirmed the reception using the `ACKJOB` command or another Discnt node sent a `SETACK` message for the job.
...
and how the cluster replicates jobs across different Discnt nodes.
...
This part of the state machine documents how Discnt nodes put a given job
...
* Discnt is new code, not tested, and will require quite some time to reach production quality. It is likely very buggy and may contain wrong assumptions or tradeoffs.
* As long as the software is non stable, the API may change in random ways without prior notification.
* It is possible that Discnt spends too much effort in approximating single delivery during failures. The **fast acknowledge** concept and command makes the user able to opt-out this efforts, but yet I may change the Discnt implementation and internals in the future if I see the user base really not caring about multiple deliveries during partitions.
* There is yet a lot of Redis dead code inside probably that could be removed.
* Discnt was designed a bit in *astronaut mode*, not triggered by an actual use case of mine, but more in response to what I was seeing people doing with Redis as a message queue and with other message queues. However I'm not an expert, if I succeeded to ship something useful for most users, this is kinda of an accomplishment. Otherwise it may just be that Discnt is pretty useless.
* As Redis, Discnt is single threaded. While in Redis there are stronger reasons to do so, in Discnt there is no manipulation of complex data structures, so maybe in the future it should be moved into a threaded server. We need to see what happens in real use cases in order to understand if it's worth it or not.
* The number of jobs in a Discnt process is limited to the amount of memory available. Again while this in Redis makes sense (IMHO), in Discnt there are definitely simple ways in order to circumvent this limitation, like logging messages on disk when the server is out of memory and consuming back the messages when memory pressure is already acceptable. However in general, like in Redis, manipulating data structures in memory is a big advantage from the point of view of the implementation simplicity and the functionality we can provide to users.
* Discnt is completely not optimized for speed, was never profiled so far. I'm currently not aware of the fact it's slow, fast, or average, compared to other messaging solutions. For sure it is not going to have Redis-alike numbers because it does a lot more work at each command. For example when a job is added, it is serialized and transmitted to other `N` servers. There is a lot more messaging passing between nodes involved, and so forth. The good news is that being totally unoptimized, there is room for improvements.
...
Discnt is a side project of Salvatore Sanfilippo, aka @antirez.
...
1. Maxmemory setting is mandatory in Discnt, and defaults to 1GB.
2. When 75% of maxmemory is reached, Discnt starts to replicate the new jobs only to external nodes, without taking a local copy, so basically if there is free RAM into other nodes, adding still works.
3. When 95% of maxmemory is reached, Discnt starts to evict data that does not violates the safety guarantees: For instance acknowledged jobs and inactive queues.
...
Yes. In Discnt it should be relatively simple to use the disk when memory is not
...
Discnt routing is not static, the cluster automatically tries to provide
...
DIStributed QUEue but is also a joke with "dis" as negation (like in *dis*order) of the strict concept of queue, since Discnt is not able to guarantee the strict ordering you expect from something called *queue*. And because of this tradeof it gains many other interesting things.
...
1. Join the `#discnt` IRC channel at **irc.freenode.net**.
2. Post on Stack Overflow using the `discnt` tag.
...
* Pivotal, for allowing me to work on Discnt, most in my spare time, but sometimes during work hours. Moreover Pivotal agreed to leave the copyright of the code to me. This is very generous. Thanks Pivotal!
* Michel Martens and Damian Janowski for providing early feedbacks about Discnt while the project was still private.
