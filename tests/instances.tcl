...
# This is used in order to test Discnt and provides
...
source ../support/redis.tcl     ; # Discnt uses the Redis protocol.
...
set ::discnt_instances {}
set ::sentinel_base_port 20000
set ::discnt_base_port 25000
...
if {[catch {cd tmp}]} {
    ...
    puts "Please run this test from the Discnt source root."
    exit 1
}
...
# Spawn a discnt instance, depending on 'type'.
...
proc pause_on_error {} {
    ...
    while 1 {
        ...
        if {$cmd eq {continue}} {
            break
        } elseif {$cmd eq {show-discnt-logs}} {
            set count 10
            if {[lindex $argv 1] ne {}} {set count [lindex $argv 1]}
            foreach_discnt_id id {
                puts "=== DISCNT $id ===="
                puts [exec tail -$count discnt_$id/log.txt]
                puts "---------------------\n"
            }
        } elseif {$cmd eq {ls}} {
            foreach_discnt_id id {
                puts -nonewline "Discnt $id"
                set errcode [catch {
                    set str {}
            ...
            puts "ls                     List Discnt instances."
            puts "show-discnt-logs \[N\] Show latest N lines of logs."
            puts "D <id> cmd ... arg     Call command in Discnt <id>."
            puts "DI <id> <field>        Show Discnt <id> INFO <field>."
...
# Command to talk with a Discnt instance.
proc D {n args} {
    set r [lindex $::discnt_instances $n]
...
proc foreach_discnt_id {idvar code} {
    set errcode [catch {uplevel 1 [list foreach_instance_id $::discnt_instances $idvar $code]} result]
    return -code $errcode $result
}
...
proc restart_instance {type id} {
    ...
    if {[server_is_up 127.0.0.1 $port 100] == 0} {
        abort_discnt_test "Problems starting $type #$id: ping timeout"
    }
